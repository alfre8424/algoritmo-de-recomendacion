...
const updateUsecase = async (user: UserEntity, password: string | null): Promise<AppError | UserEntity> => {
  ...
  try {
    ...
    await mysqlPool.connection.execute(
      `update users set name = ?, password = ?, updated_at = ?, securityQuestion = ?, securityAnswer = ? where id = ?`,
      [
        user.name ?? dbUser.name,
        encryptedPassword,
        user.updatedAt,
        user.securityQuestion ?? dbUser.securityQuestion,
        user.securityAnswer ?? dbUser.securityAnswer,
        user.id
      ]
    );
